...
const Home = () => {
    ...
    const formattedTitle = trimmedRoute.charAt(0).toUpperCase() + trimmedRoute.slice(1);
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                <Typography variant="h4" style={{ color: "var(--second-text-color)" }} className="home-title-level">
                    {formattedTitle}
                </Typography>

                <Link to={path} className="custom-link-btn play-btn">JOUER</Link>
            </motion.div>
        </AnimatePresence>
    );
}
